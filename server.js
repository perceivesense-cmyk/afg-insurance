...
// ══ 비밀번호 설정 ══════════════════════════════════════════════
// Vercel 환경변수 SITE_PASSWORD 로 설정하면 자동 적용
// 없으면 아래 기본값 사용
const SITE_PASSWORD = process.env.SITE_PASSWORD || 'afg2024';
const SESSION_HOURS = 8; // 로그인 유지 시간
...
.card{background:white;border-radius:16px;padding:40px 36px;width:360px;box-shadow:0 4px 24px rgba(0,0,0,.1)}
.logo{display:flex;align-items:center;justify-content:center;gap:12px;margin-bottom:28px}
.logo-txt{text-align:left}
.logo-txt .main{font-size:16px;font-weight:700;color:#1B2A4A}
.logo-txt .sub{font-size:11px;color:#94a3b8;margin-top:2px}
h2{font-size:18px;font-weight:700;color:#1B2A4A;margin-bottom:6px}
p{font-size:13px;color:#64748b;margin-bottom:24px}
input{width:100%;padding:12px 14px;border:1.5px solid #E2E8F0;border-radius:10px;font-size:14px;font-family:inherit;outline:none;margin-bottom:12px;transition:border-color .15s}
...
<div class="card">
  <div class="logo">
    ...
    <div class="logo-txt">
      <div class="main">보험심사평가</div>
      <div class="sub">고지의무 위반 분석 시스템</div>
    </div>
  </div>
...
app.post('/login', (req, res) => {
  ...
  sessions.set(token, { exp: Date.now() + SESSION_HOURS * 3600 * 1000 });
  res.setHeader('Set-Cookie', `afg_token=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_HOURS * 3600}`);
  res.redirect('/');
});
...
app.get('/logout', (req, res) => {
  ...
  res.setHeader('Set-Cookie', 'afg_token=; Path=/; Max-Age=0');
  res.redirect('/login');
});
...
<title>AFG 보험심사평가 — 고지의무 분석</title>
...
.hdr{background:white;border-bottom:3px solid ${NAVY};padding:0 28px;display:flex;align-items:center;justify-content:space-between;height:68px;box-shadow:0 2px 12px rgba(0,0,0,.07);position:sticky;top:0;z-index:50}
.logo{display:flex;align-items:center;gap:16px}
.logo-div{width:1px;height:42px;background:#E2E8F0}
.logo-main{font-size:16px;font-weight:700;color:${NAVY}}
.logo-sub{font-size:10px;color:#94a3b8;margin-top:2px}
.hdr-right{display:flex;gap:8px;align-items:center}
.btn-sm{padding:6px 14px;border-radius:8px;border:1px solid #E2E8F0;background:transparent;color:#64748b;font-size:12px;cursor:pointer;font-family:inherit}
.btn-sm:hover{background:#F8FAFC}
.wrap{max-width:980px;margin:0 auto;padding:24px 20px}
.notice{background:#FFFBEB;border:1px solid #FDE68A;border-radius:10px;padding:9px 16px;font-size:11px;color:#92400E;margin-bottom:16px;line-height:1.7}
.card{background:white;border-radius:14px;border:1px solid #E2E8F0;overflow:hidden;margin-bottom:14px;box-shadow:0 1px 6px rgba(0,0,0,.05)}
.card-body{padding:18px 20px}
.tabs{display:flex;border-bottom:1px solid #E2E8F0}
.tab{padding:11px 20px;border:none;background:transparent;color:#64748b;font-size:13px;cursor:pointer;border-bottom:2px solid transparent;font-family:inherit}
.tab.on{color:${NAVY};border-bottom-color:${NAVY};font-weight:700;background:#EEF6FF}
.row{display:flex;gap:9px;margin-bottom:12px;align-items:center}
input[type=text]{padding:9px 14px;border:1.5px solid #E2E8F0;border-radius:9px;font-size:13px;font-family:inherit;outline:none;background:#F8FAFC;color:#1E293B}
input[type=text]:focus{border-color:${NAVY}}
textarea{width:100%;padding:11px 14px;border:1.5px solid #E2E8F0;border-radius:10px;font-size:13px;font-family:inherit;resize:vertical;min-height:120px;outline:none;background:#F8FAFC;color:#1E293B;line-height:1.8}
textarea:focus{border-color:${NAVY}}
.btn-main{padding:11px 20px;background:${NAVY};color:white;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap}
.btn-main:disabled{background:#CBD5E1;cursor:not-allowed}
.btn-main:hover:not(:disabled){background:#243655}
.btn-sec{padding:11px 14px;background:transparent;border:1px solid #E2E8F0;color:#64748b;border-radius:10px;font-size:12px;cursor:pointer;font-family:inherit}
.btn-sec:hover{background:#F8FAFC}
.upload-zone{border:2px dashed #E2E8F0;border-radius:10px;padding:30px 20px;text-align:center;cursor:pointer;background:#F8FAFC;transition:border-color .15s}
.upload-zone:hover{border-color:${NAVY}}
.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:9px;margin:14px 0}
.scard{background:white;border:1px solid #E2E8F0;border-radius:10px;padding:12px 14px;cursor:pointer;transition:border-color .15s}
.scard:hover,.scard.on{border-color:#6366F1}
.risk-banner{border-radius:12px;padding:14px 18px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;border-width:2px;border-style:solid}
.hl-box{background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:14px 16px;font-size:13px;line-height:2.2;white-space:pre-wrap;max-height:220px;overflow-y:auto;margin-bottom:12px}
.ai-box{background:#F0F4FF;border:1px solid #C7D2FE;border-radius:10px;padding:14px 16px;font-size:13px;line-height:1.95;white-space:pre-wrap;margin-bottom:12px}
.cats{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:12px}
.cat-tag{padding:4px 11px;border-radius:20px;font-size:11px;font-weight:600;cursor:pointer;border:1px solid}
.tbl{width:100%;border-collapse:collapse;font-size:12px}
.tbl th{padding:8px 11px;text-align:left;color:#64748b;font-weight:600;border-bottom:1px solid #E2E8F0;font-size:10px;white-space:nowrap;background:#F8FAFC}
.tbl td{padding:8px 11px;border-bottom:1px solid #F1F5F9;vertical-align:top}
.tbl tr:hover td{background:#F8FAFC}
.badge{display:inline-block;padding:2px 9px;border-radius:20px;font-size:10px;font-weight:700;border:1px solid;white-space:nowrap}
.pbadge{display:inline-block;padding:1px 7px;border-radius:10px;font-size:9px;font-weight:700;border:1px solid;white-space:nowrap}
.kw-detail{margin:8px 0 12px;border-radius:10px;padding:11px 14px;font-size:12px}
.sec-hd{font-size:12px;font-weight:700;color:#475569;margin-bottom:9px}
.fil-row{display:flex;gap:5px;margin-bottom:9px;flex-wrap:wrap}
.fil-btn{padding:3px 10px;border-radius:20px;border:1px solid #E2E8F0;background:transparent;color:#64748b;font-size:11px;cursor:pointer;font-family:inherit;font-weight:600}
.fil-btn.on{background:${NAVY};border-color:${NAVY};color:white}
...
.pw-card{background:white;border-radius:16px;padding:32px;width:320px;text-align:center;box-shadow:0 8px 40px rgba(0,0,0,.2)}
.hist-panel{position:fixed;top:68px;right:0;width:300px;height:calc(100vh - 68px);background:white;border-left:1px solid #E2E8F0;z-index:40;overflow-y:auto;box-shadow:-4px 0 20px rgba(0,0,0,.08);transform:translateX(100%);transition:transform .2s}
.hist-panel.open{transform:translateX(0)}
.hist-item{padding:13px 16px;border-bottom:1px solid #F1F5F9;cursor:pointer}
.hist-item:hover{background:#F8FAFC}
@media(max-width:640px){.grid4{grid-template-columns:repeat(2,1fr)}.hdr{padding:0 14px}.wrap{padding:14px 12px}}
</style>
</head>
<body>
...
<div class="hdr">
  <div class="logo">
    <img src="${LOGO_SRC}" style="height:38px;object-fit:contain" alt="AFG">
    <div class="logo-div"></div>
    <div>
      <div class="logo-main">보험심사평가</div>
      <div class="logo-sub">고지의무 위반 통합 분석 · 금감원 2024 · HIRA KCD</div>
    </div>
  </div>
  <div class="hdr-right">
    <button class="btn-sm" onclick="toggleHist()">🗂 히스토리</button>
    <button class="btn-sm" onclick="location.href='/logout'">로그아웃</button>
  </div>
</div>
...
<div class="hist-panel" id="histPanel">
  <div style="padding:14px 16px;border-bottom:1px solid #E2E8F0;display:flex;justify-content:space-between;align-items:center">
    <span style="font-weight:700;font-size:14px">심사 히스토리</span>
    <button onclick="toggleHist()" style="background:transparent;border:none;font-size:18px;cursor:pointer;color:#94a3b8">✕</button>
  </div>
  <div id="histList"><div style="padding:28px;text-align:center;color:#94a3b8;font-size:13px">아직 심사 이력이 없습니다</div></div>
</div>
...
<div class="wrap">
  ...
  <div class="notice">
    ...
    <span style="background:#D1FAE5;color:#065F46;padding:1px 7px;border-radius:8px">3개월: 진단·치료·투약</span> &nbsp;
    <span style="background:#DBEAFE;color:#1E40AF;padding:1px 7px;border-radius:8px">1년: 추가검사 권고</span> &nbsp;
    <span style="background:#FEF3C7;color:#92400E;padding:1px 7px;border-radius:8px">5년: 7일이상치료·30일이상투약·수술·입원</span> &nbsp;
    <span style="background:#FEE2E2;color:#991B1B;padding:1px 7px;border-radius:8px">항상: 10대질병·암·에이즈</span>
  </div>
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="row">
        <input type="text" id="pname" placeholder="피심사자 성명 (선택)" style="max-width:180px">
        ...
        <button class="btn-sec" id="reportBtn" style="display:none" onclick="doReport()">📄 리포트 출력</button>
      </div>
      <div id="textPane">
        <textarea id="inputTxt" placeholder="병력 정보를 입력하세요. 질병명·약품명·수술명·검사결과 모두 입력할수록 정확도 향상.&#10;예) 2019년 고혈압 진단, 발살탄 투약 중 / 2021년 2형당뇨 / 2023년 위선암 내시경절제술 / 추간판탈출증 요추..." oninput="checkBtn()"></textarea>
      </div>
      <div id="filePane" style="display:none">
        <div class="upload-zone" onclick="document.getElementById('fileInput').click()">
          ...
          <div style="font-size:11px;color:#94a3b8;margin-top:3px">TXT · PDF 지원 (🔒 비밀번호 PDF도 가능)</div>
        </div>
  ...
  <div class="pw-modal" id="pwModal" style="display:none">
    <div class="pw-card">
      ...
      <div style="font-weight:700;font-size:15px;margin-bottom:5px">PDF 비밀번호 입력</div>
      <div id="pwFileName" style="font-size:11px;color:#94a3b8;margin-bottom:16px"></div>
      <input type="password" id="pwInput" placeholder="비밀번호" style="width:100%;padding:10px 14px;border:1.5px solid #E2E8F0;border-radius:9px;font-size:14px;font-family:inherit;outline:none;margin-bottom:10px" onkeydown="if(event.key==='Enter')confirmPw()">
  ...
  <div id="results" style="display:none">
    <div class="risk-banner" id="riskBanner">
      <div>
        <div style="font-size:11px;color:#64748b;margin-bottom:2px">전체 보험 통합 위반 위험도</div>
        <div style="font-size:22px;font-weight:700" id="riskVal">—</div>
        <div style="font-size:11px;color:#64748b;margin-top:2px" id="riskSub"></div>
      </div>
      <button class="btn-main" onclick="doReport()">📄 AFG 리포트 출력</button>
    </div>
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div style="display:flex;gap:10px;margin-top:4px;flex-wrap:wrap">
          <span style="font-size:10px;color:#94a3b8;display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:50%;background:#A855F7;display:inline-block"></span>필수고지</span>
          <span style="font-size:10px;color:#94a3b8;display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:50%;background:#EF4444;display:inline-block"></span>고위험</span>
          <span style="font-size:10px;color:#94a3b8;display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:50%;background:#F59E0B;display:inline-block"></span>중위험</span>
          <span style="font-size:10px;color:#94a3b8;display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:50%;background:#3B82F6;display:inline-block"></span>저위험</span>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-body">
        <div class="sec-hd" id="aiLabel">🤖 AI 통합 심사 의견</div>
  ...
  <div class="empty" id="emptyState">
    ...
    <div style="font-size:14px;font-weight:600;color:#475569;margin-bottom:5px">병력 정보를 입력하고 분석을 시작하세요</div>
    <div style="font-size:11px">금감원 2024 · HIRA KCD 기반 · 160개 항목 · alias 400개+ 완전 매칭</div>
  </div>
...
const NAVY='${NAVY}';
const SEV={
  critical:{label:'필수고지',bg:'#F3E8FF',border:'#A855F7',text:'#6B21A8',dot:'#A855F7'},
  high:{label:'고위험',bg:'#FEE2E2',border:'#EF4444',text:'#991B1B',dot:'#EF4444'},
  medium:{label:'중위험',bg:'#FEF3C7',border:'#F59E0B',text:'#92400E',dot:'#F59E0B'},
  low:{label:'저위험',bg:'#DBEAFE',border:'#3B82F6',text:'#1E40AF',dot:'#3B82F6'},
};
const PLABEL={M3:'3개월',Y1:'1년',Y5:'5년',ALWAYS:'항상'};
const PCOLOR={M3:'#10B981',Y1:'#3B82F6',Y5:'#F59E0B',ALWAYS:'#EF4444'};

const DB=[
  {k:'암',a:['악성종양','악성신생물','암종','암진단'],c:'10대필수',s:'critical',p:'ALWAYS',kcd:'C00-C97',d:'모든 암 — 10대 필수 고지. 부위·병기·치료 완료 여부 확인'},
  ...
  {k:'협심증',a:['불안정협심증','안정협심증','변이형협심증','가슴통증'],c:'10대필수',s:'critical',p:'ALWAYS',kcd:'I20',d:'10대 필수. 불안정형 여부·스텐트 시술 확인'},
  ...
  {k:'치매',a:['알츠하이머','혈관성치매','인지장애'],c:'뇌·신경계',s:'high',p:'Y5',kcd:'F00-F03,G30',d:'원인 구분·진행 단계·MMSE 결과 확인'},
  {k:'파킨슨',a:['파킨슨병','파킨슨증후군','PD'],c:'뇌·신경계',s:'high',p:'Y5',kcd:'G20-G21',d:'진행 단계·레보도파 투약 확인'},
  {k:'간질',a:['뇌전증','경련','경련발작','뇌전증발작'],c:'뇌·신경계',s:'high',p:'Y5',kcd:'G40-G41',d:'발작 빈도·현재 투약·운전면허 제한 여부 확인'},
  ...
  {k:'뇌종양',a:['뇌암','교모세포종','수막종'],c:'뇌·신경계',s:'high',p:'ALWAYS',kcd:'C71,D33',d:'양성·악성 구분·치료 경과·재발 여부 확인'},
  ...
  {k:'대동맥류',a:['복부대동맥류','흉부대동맥류','AAA'],c:'순환기계',s:'high',p:'Y5',kcd:'I71',d:'크기·수술 여부·파열 위험 확인'},
  ...
  {k:'용종',a:['대장용종','위용종','폴립','선종'],c:'종양·암',s:'medium',p:'Y5',kcd:'D12,K31.7',d:'악성화 위험도·제거 여부 확인'},
  {k:'조직검사',a:['생검','바이옵시','biopsy','세침흡인'],c:'종양·암',s:'medium',p:'Y5',kcd:'-',d:'결과 및 확진 여부 반드시 확인'},
  ...
  {k:'비만',a:['고도비만','병적비만'],c:'내분비·대사',s:'medium',p:'Y5',kcd:'E66',d:'BMI 30 이상 시 가중 — 수술(위밴드·위절제) 여부 확인'},
  ...
  {k:'췌장염',a:['급성췌장염','만성췌장염'],c:'소화기계',s:'medium',p:'Y5',kcd:'K85-K86',d:'만성화 여부·당뇨 합병 확인'},
  ...
  {k:'결핵',a:['폐결핵','활동성결핵','잠복결핵'],c:'호흡기계',s:'medium',p:'Y5',kcd:'A15-A19',d:'완치 여부·치료 기간·다제내성 여부 확인'},
  {k:'기흉',a:['자연기흉'],c:'호흡기계',s:'medium',p:'Y5',kcd:'J93',d:'재발성 여부·수술(흉강경) 이력 확인'},
  {k:'수면무호흡',a:['OSA','OSAS','폐쇄성수면무호흡'],c:'호흡기계',s:'medium',p:'Y5',kcd:'G47.3',d:'AHI 수치·CPAP 사용 여부·비만 동반 확인'},
  ...
  {k:'신장결석',a:['요석','요로결석','콩팥결석'],c:'비뇨기·신장',s:'low',p:'Y5',kcd:'N20',d:'재발성 여부·ESWL·수술 이력 확인'},
  {k:'디스크',a:['추간판탈출증','HIVD','목디스크','허리디스크','추간판'],c:'근골격계',s:'low',p:'Y5',kcd:'M50-M51',d:'경추·요추 구분·수술 여부·신경 손상 확인'},
  {k:'척추관협착증',a:['요추관협착','경추관협착'],c:'근골격계',s:'medium',p:'Y5',kcd:'M48.0',d:'수술 여부·보행 장애 확인'},
  {k:'골다공증',a:['골감소증','뼈엉성증'],c:'근골격계',s:'low',p:'Y5',kcd:'M80-M81',d:'T-score·골절 이력·비스포스포네이트 투약 확인'},
  ...
  {k:'망막질환',a:['망막박리','당뇨망막병증','망막혈관폐쇄'],c:'안과',s:'medium',p:'Y5',kcd:'H33-H36',d:'수술 이력·시력 손상·당뇨 합병 확인'},
  {k:'자궁근종',a:['자궁평활근종','자궁섬유종'],c:'여성질환',s:'low',p:'Y5',kcd:'D25',d:'크기·위치·수술(자궁근종절제·자궁절제) 여부 확인'},
  ...
  {k:'유방결절',a:['유방혹','유방낭종','섬유선종','BI-RADS'],c:'여성질환',s:'medium',p:'Y5',kcd:'N60,D24',d:'BI-RADS 분류·조직검사 결과·악성 감별 확인'},
  ...
  {k:'수술이력',a:['수술','외과수술','복강경수술','개복술','개흉술'],c:'수술·입원',s:'medium',p:'Y5',kcd:'-',d:'금감원: 5년 내 수술 전체 고지 — 종류·날짜·완치 여부'},
  {k:'입원이력',a:['입원','장기입원','7일이상입원'],c:'수술·입원',s:'medium',p:'Y5',kcd:'-',d:'금감원: 5년 내 7일 이상 치료·입원 — 사유·기간 필수'},
  {k:'항암치료',a:['항암','항암제','화학요법'],c:'수술·입원',s:'critical',p:'ALWAYS',kcd:'Z51.1',d:'항암 치료 이력 — 암 고지와 연계 — 종류·완료 여부 확인'},
  {k:'방사선치료',a:['방사선요법','방사선조사','IMRT','SBRT'],c:'수술·입원',s:'critical',p:'ALWAYS',kcd:'Z51.0',d:'치료 부위·암 관련 여부·완료 여부 확인'},
...
let currentFound=[],currentText='',selectedKw=null,filterSev='all',filterCat='all',history=[],pendingFile=null,histOpen=false;

function switchTab(t,el){
  document.querySelectorAll('.tab').forEach(b=>b.classList.remove('on'));el.classList.add('on');
  document.getElementById('textPane').style.display=t==='text'?'block':'none';
  document.getElementById('filePane').style.display=t==='file'?'block':'none';
}
...
function doReset(){
  document.getElementById('inputTxt').value='';document.getElementById('pname').value='';
  document.getElementById('fileLabel').textContent='클릭하여 파일 업로드';
  document.getElementById('results').style.display='none';document.getElementById('emptyState').style.display='block';
  document.getElementById('reportBtn').style.display='none';document.getElementById('analyzeBtn').disabled=true;
  currentFound=[];currentText='';selectedKw=null;
}

async function handleFile(e){
  const file=e.target.files[0];if(!file)return;
  document.getElementById('fileLabel').textContent=file.name;
  if(file.name.toLowerCase().endsWith('.pdf')){await processPdf(file,'');}
  else{const t=await file.text();document.getElementById('inputTxt').value=t;checkBtn();}
  e.target.value='';
}
async function processPdf(file,pw){
  try{
    if(!window.pdfjsLib)throw new Error('PDF.js 로딩 중');
    window.pdfjsLib.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js';
    const buf=await file.arrayBuffer();const params={data:buf};if(pw)params.password=pw;
    const pdf=await window.pdfjsLib.getDocument(params).promise;
    let out='';for(let i=1;i<=pdf.numPages;i++){const pg=await pdf.getPage(i);const ct=await pg.getTextContent();out+=ct.items.map(it=>it.str).join(' ')+'\n';}
    document.getElementById('inputTxt').value=out;document.getElementById('pwModal').style.display='none';pendingFile=null;checkBtn();
  }catch(err){
    const m=err.message||'';
    if(m.toLowerCase().includes('password')||m.toLowerCase().includes('encrypted')){
      pendingFile=file;document.getElementById('pwFileName').textContent=file.name;
      document.getElementById('pwModal').style.display='flex';document.getElementById('pwInput').value='';
      setTimeout(()=>document.getElementById('pwInput').focus(),100);
    }else{alert('PDF 오류: '+m);}
  }
}
function confirmPw(){if(pendingFile)processPdf(pendingFile,document.getElementById('pwInput').value);}
function closePw(){document.getElementById('pwModal').style.display='none';pendingFile=null;}

function findMatches(text){
  const found=new Map();
  DB.forEach(kw=>{const targets=[kw.k,...(kw.a||[])];for(const t of targets){if(text.includes(t)){if(!found.has(kw.k))found.set(kw.k,{...kw,matched:t});break;}}});
  return [...found.values()];
}
...
function hl(text,matches){
  ...
  pos.forEach(({start,end,kw,alias,sev})=>{
    if(start<cursor)return;if(start>cursor)out+=esc(text.slice(cursor,start));
    const c=SEV[sev];
    out+=`<mark style="background:${c.bg};border:1.5px solid ${c.border};border-radius:4px;padding:1px 5px;color:${c.text};font-weight:600;cursor:pointer" onclick="selectKw('${kw}')">${esc(alias)}</mark>`;
    cursor=end;
  });
  if(cursor<text.length)out+=esc(text.slice(cursor));return out;
}
function selectKw(kw){selectedKw=selectedKw===kw?null:kw;renderDetail();renderTable();}
function renderDetail(){
  const d=document.getElementById('kwDetail');
  if(!selectedKw){d.style.display='none';return;}
  const item=DB.find(x=>x.k===selectedKw);if(!item){d.style.display='none';return;}
  const c=SEV[item.s];
  d.innerHTML=`<div class="kw-detail" style="background:${c.bg};border:1.5px solid ${c.border}">
    <div style="display:flex;align-items:center;gap:7px;margin-bottom:5px">
      <strong style="color:${c.text};font-size:13px">${item.k}</strong>
      <span class="badge" style="background:${c.bg};border-color:${c.border};color:${c.text}">${c.label}</span>
      <span class="pbadge" style="border-color:${PCOLOR[item.p]};color:${PCOLOR[item.p]}">${PLABEL[item.p]}</span>
      ${item.kcd?`<span style="font-size:10px;color:${c.text};opacity:.7">KCD: ${item.kcd}</span>`:''}
    </div>
    ${item.a?.length?`<div style="font-size:10px;color:${c.text};opacity:.8;margin-bottom:4px">동의어: ${item.a.join(', ')}</div>`:''}
    <div style="font-size:12px;color:${c.text}">${item.d}</div>
  </div>`;
  d.style.display='block';
}
function renderTable(){
  const filtered=currentFound.filter(f=>(filterSev==='all'||f.s===filterSev)&&(filterCat==='all'||f.c===filterCat));
  document.getElementById('tblBody').innerHTML=filtered.map(item=>{
    const c=SEV[item.s];
    return `<tr style="${selectedKw===item.k?'background:#F0F4FF':''};cursor:pointer" onclick="selectKw('${item.k}')">
      <td style="font-weight:600">${item.k}${item.matched!==item.k?`<div style="font-size:9px;color:#94a3b8">${item.matched}</div>`:''}</td>
      <td style="color:#64748b;font-size:11px">${item.c}</td>
      <td><span class="badge" style="background:${c.bg};border-color:${c.border};color:${c.text}">${c.label}</span></td>
      <td><span class="pbadge" style="border-color:${PCOLOR[item.p]};color:${PCOLOR[item.p]}">${PLABEL[item.p]}</span></td>
      <td style="color:#94a3b8;font-size:10px">${item.kcd||'-'}</td>
      <td style="color:#475569;font-size:11px">${item.d}</td>
    </tr>`;
  }).join('');
  document.getElementById('tblLabel').textContent='📋 발견 항목 상세 ('+filtered.length+'/'+currentFound.length+'건)';
}
function renderHist(){
  const list=document.getElementById('histList');
  if(!history.length){list.innerHTML='<div style="padding:24px;text-align:center;color:#94a3b8;font-size:13px">아직 심사 이력이 없습니다</div>';return;}
  list.innerHTML=history.map((h,i)=>`<div class="hist-item" onclick="loadHist(${i})">
    <div style="display:flex;justify-content:space-between;margin-bottom:3px">
      <span style="font-size:13px;font-weight:600">${h.pname||'고객'} · ${h.count}건</span>
      <span style="font-size:10px;color:#94a3b8">${h.time}</span>
    </div>
    <div style="font-size:11px;color:#94a3b8;margin-bottom:4px">${h.preview}</div>
    <div style="display:flex;gap:4px">
      ${h.critC>0?`<span style="background:#F3E8FF;color:#6B21A8;border-radius:20px;padding:1px 7px;font-size:9px;font-weight:700">필수 ${h.critC}</span>`:''}
      <span style="background:#FEE2E2;color:#991B1B;border-radius:20px;padding:1px 7px;font-size:9px;font-weight:700">고 ${h.highC}</span>
    </div>
  </div>`).join('');
}
function loadHist(i){
  const h=history[i];
  document.getElementById('inputTxt').value=h.text;
  document.getElementById('pname').value=h.pname||'';
  currentFound=h.found;currentText=h.text;
  renderResults(h.found,h.text,h.ai);
  toggleHist();
}

async function doAnalyze(){
  const text=document.getElementById('inputTxt').value.trim();if(!text)return;
  currentText=text;currentFound=findMatches(text);selectedKw=null;filterSev='all';filterCat='all';
  renderResults(currentFound,text,'분석 중...');
  // AI
  const pname=document.getElementById('pname').value||'고객';
  const critItems=currentFound.filter(f=>f.s==='critical');
  const prompt='당신은 보험심사평가사입니다. 금감원 표준약관 2024 기준으로 분석해 주세요.\n피심사자: '+pname+'\n발견 필수고지: '+critItems.map(f=>f.k).join(', ')||'없음'+'\n전체: '+currentFound.map(f=>f.k).join(', ')||'없음'+'\n병력:\n'+text+'\n\n형식:\n【종합 위험 평가】(2~3문장)\n【보험 유형별 영향】\n• 생명보험:\n• 실손보험:\n• 암보험:\n• 치매보험:\n• 간편보험(333) 가입 여부:\n【핵심 심사 포인트】(3~5가지, 고지기간 명시)\n【심사관 권고사항】(1~3가지)\n전문적이고 간결한 한국어로.';
  try{
    ...
    history.unshift({pname,text,found:currentFound,ai,count:currentFound.length,critC:currentFound.filter(f=>f.s==='critical').length,highC:currentFound.filter(f=>f.s==='high').length,preview:text.slice(0,50)+'…',time:ts});
    if(history.length>15)history.pop();
    renderHist();
  }catch(e){document.getElementById('aiBox').textContent='AI 분석 오류: '+e.message;}
}

function renderResults(found,text,ai){
  const critC=found.filter(f=>f.s==='critical').length;
  const highC=found.filter(f=>f.s==='high').length;
  const medC=found.filter(f=>f.s==='medium').length;
  const lowC=found.filter(f=>f.s==='low').length;
  const total=found.length;
  const riskLabel=critC>0?'최고위험':highC>=3?'매우 높음':highC>=1?'높음':medC>=3?'보통':'낮음';
  const riskColor=critC>0?'#A855F7':highC>=3?'#EF4444':highC>=1?'#F97316':medC>=1?'#F59E0B':'#10B981';
  document.getElementById('emptyState').style.display='none';
  document.getElementById('results').style.display='block';
  document.getElementById('reportBtn').style.display='inline-block';
  const banner=document.getElementById('riskBanner');
  banner.style.borderColor=riskColor;banner.style.boxShadow='0 0 0 3px '+riskColor+'22';
  banner.style.background=riskColor+'0D';
  document.getElementById('riskVal').textContent=riskLabel;document.getElementById('riskVal').style.color=riskColor;
  ...
  document.getElementById('countGrid').innerHTML=[
    {label:'필수고지',count:critC,key:'critical'},{label:'고위험',count:highC,key:'high'},
    {label:'중위험',count:medC,key:'medium'},{label:'저위험',count:lowC,key:'low'},
  ].map(it=>{
    const c=SEV[it.key];
    return '<div class="scard" onclick="toggleSev(\''+it.key+'\')"><div style="display:flex;align-items:center;gap:5px;margin-bottom:4px"><span style="width:8px;height:8px;border-radius:50%;background:'+c.dot+';display:inline-block"></span><span style="font-size:10px;color:#64748b">'+it.label+'</span></div><div style="font-size:20px;font-weight:700;color:'+c.text+'">'+it.count+'</div></div>';
  }).join('');
  const cats=[...new Set(found.map(f=>f.c))];
  document.getElementById('catTags').innerHTML=cats.map(cat=>{
    const items=found.filter(f=>f.c===cat);
    const top=items.some(f=>f.s==='critical')?'critical':items.some(f=>f.s==='high')?'high':items.some(f=>f.s==='medium')?'medium':'low';
    const c=SEV[top];
    return '<span class="cat-tag" style="background:'+c.bg+';border-color:'+c.border+';color:'+c.text+'" onclick="toggleCat(\''+cat+'\')">'+cat+' '+items.length+'</span>';
  }).join('');
  document.getElementById('hlBox').innerHTML=hl(text,found);
  document.getElementById('kwDetail').style.display='none';
  document.getElementById('aiBox').textContent=ai;
  document.getElementById('sevFilters').innerHTML=['all','critical','high','medium','low'].map(s=>'<button class="fil-btn'+(filterSev===s?' on':'')+'" onclick="setSev(\''+s+'\')">'+( s==='all'?'전체':SEV[s].label)+'</button>').join('');
  renderTable();
}
function toggleSev(key){filterSev=filterSev===key?'all':key;renderTable();}
function setSev(s){filterSev=s;document.querySelectorAll('.fil-btn').forEach((b,i)=>{const vals=['all','critical','high','medium','low'];b.classList.toggle('on',vals[i]===s);});renderTable();}
function toggleCat(cat){filterCat=filterCat===cat?'all':cat;renderTable();}

function doReport(){
  const pname=document.getElementById('pname').value||'고객';
  const ts=new Date().toLocaleString('ko-KR');
  const aiTxt=document.getElementById('aiBox').textContent;
  const critC=currentFound.filter(f=>f.s==='critical').length;
  const highC=currentFound.filter(f=>f.s==='high').length;
  const medC=currentFound.filter(f=>f.s==='medium').length;
  const lowC=currentFound.filter(f=>f.s==='low').length;
  const riskLabel=critC>0?'최고위험':highC>=3?'매우 높음':highC>=1?'높음':medC>=3?'보통':'낮음';
  const riskColor=critC>0?'#A855F7':highC>=3?'#EF4444':highC>=1?'#F97316':medC>=1?'#F59E0B':'#10B981';
  const sevGroups=[
    {items:currentFound.filter(f=>f.s==='critical'),c:SEV.critical,label:'🟣 필수고지 (10대질병)'},
    {items:currentFound.filter(f=>f.s==='high'),c:SEV.high,label:'🔴 고위험'},
    {items:currentFound.filter(f=>f.s==='medium'),c:SEV.medium,label:'🟡 중위험'},
    {items:currentFound.filter(f=>f.s==='low'),c:SEV.low,label:'🔵 저위험'},
  ].filter(g=>g.items.length>0);
  const cols=sevGroups.length===1?'1fr':sevGroups.length===2?'1fr 1fr':'1fr 1fr';
  const LOGO_SRC2='${LOGO_SRC}';
  const html='<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>보험심사평가 리포트 — '+pname+'</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:\'Apple SD Gothic Neo\',\'Malgun Gothic\',sans-serif;background:#f0f4f8;color:#1e293b;font-size:13px}@media print{body{background:white}.no-print{display:none}@page{margin:10mm 15mm}}</style></head><body><div style="max-width:900px;margin:0 auto;padding:20px"><div style="background:white;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1);margin-bottom:16px"><div style="padding:18px 28px 14px;border-bottom:3px solid #1B2A4A;display:flex;align-items:center;justify-content:space-between"><div style="display:flex;align-items:center;gap:16px"><img src="'+LOGO_SRC2+'" style="height:40px;object-fit:contain" alt="AFG"><div style="width:1px;height:38px;background:#e2e8f0"></div><div><div style="font-size:18px;font-weight:700;color:#1B2A4A">보험심사평가</div><div style="font-size:11px;color:#64748b;margin-top:2px">고지의무 위반 종합 분석 리포트 · 금감원 표준약관 2024</div></div></div><div style="text-align:right"><div style="font-size:10px;color:#94a3b8">심사일시</div><div style="font-size:12px;font-weight:700">'+ts+'</div><div style="margin-top:5px;display:inline-block;background:#1B2A4A;color:white;font-size:9px;padding:3px 9px;border-radius:4px">보험심사평가사 작성</div></div></div><div style="background:#fefce8;padding:8px 28px;border-bottom:1px solid #fde68a;font-size:11px;color:#92400e">⚠️ 3개월: 진단·치료·투약 / 1년: 추가검사 / 5년: 7일이상치료·30일이상투약·수술·입원 / 10대질병: 항상</div><div style="padding:18px 28px;display:flex;gap:14px;border-bottom:1px solid #f1f5f9"><div style="flex:1;background:#f8fafc;border-radius:10px;padding:14px 18px;border:1px solid #e2e8f0"><div style="font-size:10px;color:#64748b;margin-bottom:4px">피심사자</div><div style="font-size:22px;font-weight:700;color:#0f172a">'+pname+' 님</div><div style="margin-top:8px;font-size:11px;color:#64748b">총 <strong>'+currentFound.length+'건</strong> (필수 '+critC+' / 고 '+highC+' / 중 '+medC+' / 저 '+lowC+')</div></div><div style="flex:1;background:#f8fafc;border-radius:10px;padding:14px 18px;border:2px solid '+riskColor+';text-align:center"><div style="font-size:10px;color:#64748b;margin-bottom:4px">종합 위반 위험도</div><div style="font-size:28px;font-weight:700;color:'+riskColor+'">'+riskLabel+'</div></div></div><div style="padding:14px 28px;border-bottom:1px solid #f1f5f9"><div style="font-size:12px;font-weight:700;margin-bottom:12px">[ 보험 고지의무 해당 항목 ]</div><div style="display:grid;grid-template-columns:'+cols+';gap:10px">'+sevGroups.map(g=>'<div style="background:'+g.c.bg+';border:1.5px solid '+g.c.border+';border-radius:9px;padding:12px"><div style="font-size:10px;font-weight:700;color:'+g.c.text+';margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid '+g.c.border+'">'+g.label+' ('+g.items.length+'건)</div>'+g.items.map(f=>'<div style="margin-bottom:6px;line-height:1.5"><strong style="font-size:11px;color:'+g.c.text+'">• '+f.k+'</strong>'+(f.matched!==f.k?'<span style="font-size:9px;color:'+g.c.text+';opacity:.7"> ('+f.matched+')</span>':'')+'<br><span style="font-size:10px;color:'+g.c.text+';opacity:.8">'+f.d+'</span></div>').join('')+'</div>').join('')+'</div></div>'+(aiTxt&&aiTxt!=='분석 중...'?'<div style="padding:14px 28px;border-bottom:1px solid #f1f5f9"><div style="font-size:12px;font-weight:700;margin-bottom:8px">AI 통합 심사 의견</div><div style="background:#f0f4ff;border:1px solid #c7d2fe;border-radius:9px;padding:12px 16px;font-size:12px;line-height:1.9;white-space:pre-wrap">'+aiTxt+'</div></div>':'')+'<div style="padding:14px 28px;border-bottom:1px solid #f1f5f9"><div style="font-size:12px;font-weight:700;margin-bottom:10px">발견 항목 상세 (KCD 포함)</div><table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr style="background:#f8fafc"><th style="padding:6px 9px;text-align:left;color:#64748b;border-bottom:1px solid #e2e8f0">항목</th><th style="padding:6px 9px;text-align:left;color:#64748b;border-bottom:1px solid #e2e8f0">분류</th><th style="padding:6px 9px;text-align:left;color:#64748b;border-bottom:1px solid #e2e8f0">위험도</th><th style="padding:6px 9px;text-align:left;color:#64748b;border-bottom:1px solid #e2e8f0">고지기간</th><th style="padding:6px 9px;text-align:left;color:#64748b;border-bottom:1px solid #e2e8f0">KCD</th><th style="padding:6px 9px;text-align:left;color:#64748b;border-bottom:1px solid #e2e8f0">심사 포인트</th></tr></thead><tbody>'+currentFound.map(f=>'<tr><td style="padding:6px 9px;font-weight:700;border-bottom:1px solid #f1f5f9">'+f.k+(f.matched!==f.k?'<div style="font-size:9px;color:#94a3b8">'+f.matched+'</div>':'')+'</td><td style="padding:6px 9px;color:#64748b;border-bottom:1px solid #f1f5f9">'+f.c+'</td><td style="padding:6px 9px;border-bottom:1px solid #f1f5f9"><span style="background:'+SEV[f.s].bg+';color:'+SEV[f.s].text+';border:1px solid '+SEV[f.s].border+';border-radius:20px;padding:1px 8px;font-size:9px;font-weight:700">'+SEV[f.s].label+'</span></td><td style="padding:6px 9px;border-bottom:1px solid #f1f5f9"><span style="border:1px solid '+PCOLOR[f.p]+';color:'+PCOLOR[f.p]+';border-radius:10px;padding:0 6px;font-size:9px;font-weight:700">'+PLABEL[f.p]+'</span></td><td style="padding:6px 9px;color:#94a3b8;border-bottom:1px solid #f1f5f9;font-size:10px">'+(f.kcd||'-')+'</td><td style="padding:6px 9px;color:#475569;border-bottom:1px solid #f1f5f9">'+f.d+'</td></tr>').join('')+'</tbody></table></div><div style="padding:12px 28px;background:#f8fafc;display:flex;align-items:center;justify-content:space-between"><span style="font-size:10px;color:#94a3b8">AFG Authentic · 금감원 2024 · HIRA KCD</span><span style="font-size:10px;color:#94a3b8">'+ts+'</span></div></div><div class="no-print" style="text-align:center;padding-bottom:20px"><button onclick="window.print()" style="padding:11px 30px;background:#1B2A4A;color:white;border:none;border-radius:9px;font-size:13px;cursor:pointer;margin-right:8px">🖨️ 인쇄 / PDF 저장</button><button onclick="window.close()" style="padding:11px 18px;background:transparent;color:#64748b;border:1px solid #e2e8f0;border-radius:9px;font-size:13px;cursor:pointer">닫기</button></div></div></body></html>';
  const w=window.open('','_blank');
  if(!w){alert('팝업이 차단됐습니다. 허용 후 다시 시도하세요.');return;}
...
app.listen(PORT, () => console.log('AFG 보험심사평가 서버 실행 중: http://localhost:' + PORT));
module.exports = app;
